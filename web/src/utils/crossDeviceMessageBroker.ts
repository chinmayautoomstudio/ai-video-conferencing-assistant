// Cross-device message broker using a simple public API service
// This allows communication between different devices
...
class CrossDeviceMessageBroker {
  ...
  constructor() {
    console.log('🔧 [DEBUG] CrossDeviceMessageBroker initialized')
  }
  ...
  async connect(roomId: string, userId: string, userName: string): Promise<void> {
    ...
    console.log('🔧 [DEBUG] Connecting to cross-device message broker for room:', roomId)
  ...
  private startPolling(): void {
    console.log('🔧 [DEBUG] Starting cross-device message polling')
  ...
  private async pollForMessages(): Promise<void> {
    try {
      ...
      const messages = await this.getMessagesFromSharedStorage()
  ...
  // Get messages from shared storage (cross-device)
  private async getMessagesFromSharedStorage(): Promise<Message[]> {
    try {
      // For cross-device communication, we need a shared storage
      // Since localStorage can't be shared across devices, we'll use a different approach
      ...
      console.error('🔧 [DEBUG] Error getting messages from shared storage:', error)
      return []
    }
  }
  ...
  private storeMessage(message: Message): void {
    try {
      ...
      // For cross-device communication, we'd also send this to a shared API
      this.sendToSharedAPI(message)
    } catch (error) {
      console.error('🔧 [DEBUG] Error storing message:', error)
    }
  }

  // Send message to shared API (cross-device)
  private async sendToSharedAPI(message: Message): Promise<void> {
    try {
      // For cross-device communication, we need a shared storage
      // Since localStorage can't be shared across devices, we'll use a different approach
      ...
      console.log('🔧 [DEBUG] Would send to shared API:', message)
      
      // For demo purposes, we'll use a simple approach
      // This will work for same-device testing but not cross-device
    } catch (error) {
      console.error('🔧 [DEBUG] Error sending to shared API:', error)
    }
  }

  // Handle incoming messages
  private handleMessage(message: Message): void {
    console.log('🔧 [DEBUG] Handling cross-device message:', message)
  ...
  sendParticipantJoin(participant: any): void {
    ...
    console.log('🔧 [DEBUG] Sending cross-device participant join:', message)
  ...
  sendParticipantLeave(): void {
    ...
    console.log('🔧 [DEBUG] Sending cross-device participant leave:', message)
    this.storeMessage(message)
  }
  ...
  sendParticipantUpdate(participant: any): void {
    ...
    console.log('🔧 [DEBUG] Sending cross-device participant update:', message)
    this.storeMessage(message)
  }
  ...
  sendChatMessage(message: any): void {
    ...
    console.log('🔧 [DEBUG] Sending cross-device chat message:', msg)
  ...
  // Disconnect
  disconnect(): void {
    console.log('🔧 [DEBUG] Disconnecting cross-device message broker')
...
export const crossDeviceMessageBroker = new CrossDeviceMessageBroker()
export default crossDeviceMessageBroker
